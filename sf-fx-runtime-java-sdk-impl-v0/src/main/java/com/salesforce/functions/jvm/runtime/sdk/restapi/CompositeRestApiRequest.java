...
public class CompositeRestApiRequest<T> implements RestApiRequest<Map<String, T>> {
    ...
    public CompositeRestApiRequest(URI baseUri, String apiVersion, Map<String, RestApiRequest<T>> subrequests) {
    ...
    public Optional<JsonElement> getBody() {
        ...
        for (Map.Entry<String, RestApiRequest<T>> entry : subrequests.entrySet()) {
            ...
            // RestApiRequest#createUri return an absolute URL. For a composite request, we need to strip off the
            ...
            entry.getValue().getBody()
                    .ifPresent(jsonElement -> subrequestJson.add("body", jsonElement));
    ...
    @Override
    public Map<String, T> processResponse(int statusCode, Map<String, String> headers, JsonElement body) {
        Map<String, T> result = new HashMap<>();

        if (statusCode == 200) {
            JsonArray compositeResponses = body.getAsJsonObject().get("compositeResponse").getAsJsonArray();
            ...
            for (JsonElement response : compositeResponses) {
                ...
                Map<String, String> subrequestHeaders =
                        gson.fromJson(responseAsObject.get("httpHeaders"), new TypeToken<Map<String, String>>() {
                        }.getType());
                JsonElement subrequestBody = responseAsObject.get("body");

                T t = subrequests.get(referenceId).processResponse(subrequestStatusCode, subrequestHeaders, subrequestBody);
