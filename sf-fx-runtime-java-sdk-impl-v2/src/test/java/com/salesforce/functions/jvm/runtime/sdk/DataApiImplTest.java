...
public class DataApiImplTest {
  ...
  @Mock private SalesforceContextCloudEventExtension salesforceContext;
  @Mock private SalesforceFunctionContextCloudEventExtension functionContext;
  ...
  @Before
  public void setUp() {
    when(functionContext.getAccessToken())
        .thenReturn(
            "00DB0000000UIn2!AQMAQKXBvR03lDdfMiD6Pdpo_wiMs6LGp6dVkrwOuqiiTEmwdPb8MvSZwdPLe009qHlwjxIVa4gY.JSAd0mfgRRz22vS");

    when(salesforceContext.getUserContext()).thenReturn(userContext);

    when(userContext.getSalesforceBaseUrl()).thenReturn(URI.create("http://localhost:8080"));

    orgImpl = new OrgImpl(salesforceContext, functionContext, "53.0");

    dataApi = orgImpl.getDataApi();
  }
  ...
  public void testCreate() {
    Record result =
        orgImpl
            ...
            .build();
    // a00B000000FSkcvIAD
    assertThat(result.getStringField("Id"), is(equalTo(Optional.empty())));
  }
  ...
  public void testUpdate() {
    ...
    assertThat(result.getStringField("Id").get(), is(equalTo("a00B000000FSjVUIA1")));
  }

  @Test(expected = IllegalArgumentException.class)
  public void testUpdateWithoutIdField() throws DataApiException {
    dataApi.update(
        orgImpl.newRecordBuilder("Movie__c").withField("ReleaseDate__c", "1980-05-21").build());
  }
  ...
  public void testUnitOfWork() throws DataApiException {
    ...
    ReferenceId franchiseReference =
        unitOfWorkBuilder.registerCreate(
            orgImpl.newRecordBuilder("Franchise__c").withField("Name", "Star Wars").build());
  ...
  public void testNewRecordBuilderWithForeignRecord() {
    orgImpl.newRecordBuilder(Mockito.mock(Record.class));
  }
