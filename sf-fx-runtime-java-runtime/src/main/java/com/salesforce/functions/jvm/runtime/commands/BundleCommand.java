...
@Command(name = "bundle",
        header = "Pre-bundles a function project"
)
public class BundleCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        List<SalesforceFunction> functions = new SalesforceFunctionsProjectFunctionsScanner().scan(project);
        ...
        // Copy all dependencies from the project to the classpath directory of the function bundle directory.
        ...
        for (Path dependencyPath : project.getClasspathPaths()) {
            final Path destinationDependencyPath = getPathForDestinationDirectory(bundleClassPath, dependencyPath);

            if (Files.isDirectory(dependencyPath)) {
                Files.walkFileTree(dependencyPath, new SimpleFileVisitor<Path>() {
                    @Override
                    public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs)
                            throws IOException {
                        Files.createDirectories(destinationDependencyPath.resolve(dependencyPath.relativize(dir)));
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                        ...
                        Files.copy(file, destinationDependencyPath.resolve(dependencyPath.relativize(file)));
                        return FileVisitResult.CONTINUE;
                    }
                });
    ...
    private Path getPathForDestinationDirectory(Path destinationDirectoryPath, Path path) {
        Path destination = Paths.get(destinationDirectoryPath.toString(), path.getFileName().toString());
        while (Files.exists(destination)) {
            destination = Paths.get(destinationDirectoryPath.toString(), "_", path.getFileName().toString());
        }
