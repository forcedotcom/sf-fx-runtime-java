...
public final class JsonLibraryDetector {
    ...
    /**
     * Detects which JsonLibrary should be used with the given class. If no specific JsonLibrary is strictly required
     * for the given class, a fallback JsonLibrary will be used as a catch-all.
     *
     ...
     * @throws AmbiguousJsonLibraryException If multiple JSON libraries announce they must be used for the given class.
     */
    public static JsonLibrary detect(Class<?> clazz) throws AmbiguousJsonLibraryException {
        ClassLoader classLoader = clazz.getClassLoader();

        // getClassLoader returns null when the class was loaded by the bootstrap classloader. To avoid juggling with
        // null pointers downstream, we normalize the variable here by putting the actual bootstrap classloader in.
        if (classLoader == null) {
        ...
        try {
            ...
            // If the library is not present in the user project's classpath, it cannot be used in any case.
        ...
        for (JsonLibrary jsonLibrary : availableJsonLibraries) {
            if (jsonLibrary.mustBeUsedFor(clazz)) {
                if (responsibleJsonLibrary != null) {
                    throw new AmbiguousJsonLibraryException(String.format("Multiple JSON libraries declared responsibility for class %s!", clazz.getName()));
                }
