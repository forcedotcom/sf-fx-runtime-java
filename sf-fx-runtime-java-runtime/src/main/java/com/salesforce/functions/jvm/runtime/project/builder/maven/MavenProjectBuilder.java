...
public final class MavenProjectBuilder implements ProjectBuilder {
  ...
  private static List<Path> resolveDependencyPaths(Path projectPath)
  ...
  private static Optional<Path> resolveBuildOutputDirectory(Path projectPath)
      throws MavenInvocationException {
    // In almost all cases, the directory will be target/classes. To avoid calling Maven too often
    // (since it takes
    // a significant amount of time, especially here because the help plugin usually needs to be
    // downloaded first),
    ...
    return MavenInvoker.invoke(projectPath, "help:evaluate", properties, outputHandler)
        .map(Paths::get);
  }
